...
int optCostUtil(std::vector<int> &freq, int i, int j)
{
	...
	for (int r = i; r <= j; r++)
		min = std::min(min, optCostUtil(freq, i, r - 1) + optCostUtil(freq, r + 1, j));
...
int optCostTDUtil(std::vector<int> &freq, std::vector<std::vector<int>> &cost, int i, int j)
{
	if (i > j)
		return 0;
	...
	for (int r = i; r <= j; r++)
	{
		cost[i][j] = std::min(cost[i][j], optCostTDUtil(freq,cost, i, r - 1) + 
...
{
	int n = freq.size();
	std::vector<std::vector<int>> cost = std::vector<std::vector<int>>(n, std::vector<int>(n, std::numeric_limits<int>::max()));
	for (int i = 0; i < n; i++)
		cost[i][i] = freq[i];
...
int optCostBU(std::vector<int> &keys, std::vector<int> &freq)
{
	...
	for (int l = 1; l < n; l++) // l is length of range.
	{
		for (int i = 0, j = i + l; j < n; i++, j++)
		{
			...
			for (int r = i; r <= j; r++)
			{
				cost[i][j] = std::min(cost[i][j], sm + ((r - 1 >= i)? cost[i][r - 1] : 0) + ((r + 1 <= j)? cost[r + 1][j] : 0));
			}
		}
	}
	return cost[0][n - 1];
