...
int MatrixChainMulTDUtil(std::vector<std::vector<int>> &dp, std::vector<int> &p, int i, int j)
{
	// Base Case
	if (i == j)
		return 0;
	...
	for (int k = i; k < j; k++)
	{
		dp[i][j] = std::min(dp[i][j], MatrixChainMulTDUtil(dp, p, i, k) + 
...
int MatrixChainMulBU(std::vector<int> &p, int n)
{
	...
	{ 
		...
		{
			for (int k = i; k < j; k++)
				dp[i][j] = std::min(dp[i][j], dp[i][k] + p[i - 1] * p[k] * p[j] + dp[k + 1][j]);
		}
	}
	return dp[1][n - 1];
}
...
int main()
{
	...
	int n = arr.size();
	std::cout << "Matrix Chain Multiplication is: " << MatrixChainMulBruteForce(arr, n) << std::endl;
	std::cout << "Matrix Chain Multiplication is: " << MatrixChainMulTD(arr, n) << std::endl;
	std::cout << "Matrix Chain Multiplication is: " << MatrixChainMulBU(arr, n) << std::endl;
	return 0;
}
