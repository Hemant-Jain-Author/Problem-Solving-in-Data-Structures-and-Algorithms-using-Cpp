...
int maxVal(std::vector<std::vector<int>> &max, int i, int j)
{
	...
	for (int k = i;k < j;k++)
		max[i][j] = std::max(max[i][j], std::max(maxVal(max, i, k), maxVal(max, k + 1, j)));
	
	return max[i][j];
}

int findSumTDUtil(std::vector<std::vector<int>> &dp, std::vector<std::vector<int>> &max, int i, int j, std::vector<int> &arr)
{
	if (j <= i)
		return 0;
	...
	for (int k = i;k < j;k++)
	{
		dp[i][j] = std::min(dp[i][j], findSumTDUtil(dp, max, i, k, arr) + 
...
int findSumBU(std::vector<int> &arr)
{
	...
	std::vector<std::vector<int>> max = std::vector<std::vector<int>>(n, std::vector<int>(n, 0));
