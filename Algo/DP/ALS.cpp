...
int fastestWayBU(std::vector<std::vector<int>> &a, std::vector<std::vector<int>> &t, std::vector<int> &e, std::vector<int> &x, int n)
{
	std::vector<std::vector<int>> f = std::vector<std::vector<int>>(2, std::vector<int>(n, 0));
	// Time taken to leave first station.
	f[0][0] = e[0] + a[0][0];
	f[1][0] = e[1] + a[1][0];
	...
	for (int i = 1; i < n; ++i)
	{
		f[0][i] = std::min(f[0][i - 1] + a[0][i], f[1][i - 1] + t[1][i - 1] + a[0][i]);
		f[1][i] = std::min(f[1][i - 1] + a[1][i], f[0][i - 1] + t[0][i - 1] + a[1][i]);
	}
