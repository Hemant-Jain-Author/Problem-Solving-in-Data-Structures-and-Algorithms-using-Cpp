...
int SegmentTree::constructST(std::vector<int> &input, int start, int end, int index)
{
...
int SegmentTree::getSumUtil(int segStart, int segEnd, int queryStart, int queryEnd, int index)
{
...
int SegmentTree::setUtil(int segStart, int segEnd, int ind, int val, int index)
{
	...
	int mid = (segStart + segEnd) / 2;
	int diff = setUtil(segStart, mid, ind, val, 2 * index + 1) + setUtil(mid + 1, segEnd, ind, val, 2 * index + 2);
...
int main()
{
	...
	std::cout << "Sum of values in the range(0, 3): " << tree->getSum(1, 3) << std::endl;
	std::cout << "Sum of values of all the elements: " << tree->getSum(0, arr.size() - 1) << std::endl;
